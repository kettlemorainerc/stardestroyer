...
public class Move extends CommandBase {
  ...
  public enum Style {
	...
	private Move(double north, double east, double rotation, Style method, Subsystem... requirements) {
		...
		if (north != 0 && east != 0) {
			distanceTotal_ = new double[]{0, 0, rotation * 7 / 8};
		} else {
	...
	public void initialize() {
		...
		double[] scale = {Math.abs(distanceTotal_[NORTH]) / max[NORTH], Math.abs(distanceTotal_[EAST]) / max[EAST], Math.abs(distanceTotal_[CLOCKWISE]) / max[CLOCKWISE]};
		double maxScale = Math.max(scale[NORTH], Math.max(scale[EAST], scale[CLOCKWISE]));
		scale = new double[]{scale[NORTH] / maxScale, scale[EAST] / maxScale, scale[CLOCKWISE] / maxScale}; // NORTH - EAST
		double[] sign = {Math.signum(distanceTotal_[NORTH]), Math.signum(distanceTotal_[EAST]), Math.signum(distanceTotal_[CLOCKWISE])};
		...
		fast_ = new double[]{
				...
				Math.max(min[CLOCKWISE], max[CLOCKWISE] * scale[CLOCKWISE]) * sign[CLOCKWISE]}; // don't let maximum scale below minimum
		slow_ = new double[]{min[NORTH] * sign[NORTH], min[EAST] * sign[EAST], min[CLOCKWISE] * sign[CLOCKWISE]}; // don't scale below minimum
		...
		finished_ = new boolean[]{Math.abs(distanceRemaining_[NORTH]) == 0., Math.abs(distanceRemaining_[EAST]) == 0., Math.abs(distanceRemaining_[CLOCKWISE]) == 0.};

		System.out.println("$$$$$$$$$$$$$$$$$$ MOVECLOCKWISE DISTANCE:" + distanceTotal_[NORTH] + " " + distanceTotal_[EAST] + " " + distanceTotal_[CLOCKWISE] + " (" + method_ + ")");
		System.out.println("$$$$$$$$$$$$$$$$$$ MOVECLOCKWISE SCALE:" + scale[NORTH] + " " + scale[EAST] + " " + scale[CLOCKWISE]);
		System.out.println("$$$$$$$$$$$$$$$$$$ MOVECLOCKWISE FAST:" + fast_[NORTH] + " " + fast_[EAST] + " " + fast_[CLOCKWISE]);
		System.out.println("$$$$$$$$$$$$$$$$$$ MOVECLOCKWISE SLOW:" + slow_[NORTH] + " " + slow_[EAST] + " " + slow_[CLOCKWISE]);
		System.out.println("$$$$$$$$$$$$$$$$$$ MOVECLOCKWISE ACCEL N:" + acceleration_[NORTH][NORTH] + " " + acceleration_[NORTH][EAST]);
		System.out.println("$$$$$$$$$$$$$$$$$$ MOVECLOCKWISE ACCEL E:" + acceleration_[EAST][NORTH] + " " + acceleration_[EAST][EAST]);
		System.out.println("$$$$$$$$$$$$$$$$$$ MOVECLOCKWISE ACCEL R:" + acceleration_[CLOCKWISE][NORTH] + " " + acceleration_[CLOCKWISE][EAST]);
	}
